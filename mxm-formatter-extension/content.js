...
if (typeof mxmFormatterRoot.mxmFormatterLoaded === "undefined") {
  ...
  (function (global) {
    ...
    function isCountingSequence(line) {
      ...
      return /^\s*(?:\d{1,2}\s*[,–—.\u2026]?\s*){2,}\d{1,2}\b.*\b(?:go|down|lift.?off|let['’]?s\s*go|c'?mon|start|ready|set|and)?\b/i.test(
        line.trim(),
      );
    }
    ...
    for (const [rawKey, canonical] of Object.entries(
    ...
    function buildSmartProperMap(text) {
      ...
      const cueRe =
        /\b(feat\.?|featuring|ft\.?|prod\.?|produced|remix|remixed|mixed|mix|version|vs\.?)\b\s+([0-9A-Za-z][0-9A-Za-z.'$]*(?:\s+[0-9A-Za-z][0-9A-Za-z.'$]*){0,3})/gi;
      ...
      for (const [lower, s] of stats.entries()) {
        ...
        if (
          Object.prototype.hasOwnProperty.call(ALWAYS_PROPER_CANONICAL, lower)
        ) {
    ...
    // ---------- Formatter ----------
    function applyWordCase(match, canonical) {
    ...
    const THANK_YOU_ADDRESS_WORDS = new Set([
      "baby",
      "babe",
      "bro",
      "brother",
    ...
    const THANK_YOU_BLOCKED_FOLLOWERS = new Set([
      "a",
      "again",
      "all",
      "an",
    ...
    function isThankYouAddress(name) {
      const lower = name.toLowerCase();
      if (THANK_YOU_BLOCKED_FOLLOWERS.has(lower)) return false;
      return THANK_YOU_ADDRESS_WORDS.has(lower);
    }

    function isThankYouNounPhrase(str, offset) {
      const before = str.slice(Math.max(0, offset - 32), offset);
      return /\b(?:a|an|the|this|that|my|your|our|his|her|their)\s+$/i.test(
        before,
      );
    }

    function normalizeThankYouName(name) {
      const lower = name.toLowerCase();
      if (lower === "john") return applyWordCase(name, "John");
      if (THANK_YOU_ADDRESS_WORDS.has(lower)) {
        if (name === name.toUpperCase()) return name.toUpperCase();
        if (name[0] === name[0].toUpperCase())
          return name[0].toUpperCase() + name.slice(1);
        return lower;
      }
      return applyWordCase(name, lower);
    }
    ...
    function formatLyrics(input, _options = {}) {
      ...
      x = x.replace(/\bjesus\b/gi, (match) =>
        applyCasedReplacement(match, "Jesus"),
      );
      x = x.replace(/\bchrist\b/gi, (match) =>
        applyCasedReplacement(match, "Christ"),
      );
      ...
      // --- Contraction normalization block ---
      {
        ...
        for (let i = 0; i < contractionLines.length; i++) {
          ...
          line = line.replace(
            /\b(should|could|would)\s+(of)\b/gi,
            (_match, modal, ofWord) =>
              `${modal} ${applyWordCase(ofWord, "have")}`,
          );
          line = line.replace(/\byoure\b/gi, (match) =>
            applyWordCase(match, "you're"),
          );
          line = line.replace(/\btheyre\b/gi, (match) =>
            applyWordCase(match, "they're"),
          );
          line = line.replace(/\bshes\b/gi, (match) =>
            applyWordCase(match, "she's"),
          );
          line = line.replace(/\bhes\b/gi, (match) =>
            applyWordCase(match, "he's"),
          );
      ...
      x = x
        ...
        .replace(/\byoure\b/gi, (match) => applyWordCase(match, "you're"))
        .replace(/\btheyre\b/gi, (match) => applyWordCase(match, "they're"))
        .replace(/\bshes\b/gi, (match) => applyWordCase(match, "she's"))
        .replace(/\bhes\b/gi, (match) => applyWordCase(match, "he's"))
      ...
      x = x.replace(/((?:^|\n)\s*)'til\b/g, (match, boundary, offset, str) => {
        ...
        const rest =
          lineEnd === -1
            ? str.slice(afterStart)
            : str.slice(afterStart, lineEnd);
      ...
      x = x.replace(
        ...
        (m, word, off, str) => {
          ...
          if (lower === "well") {
            ...
            forms.add(noApos);
            const noSuffix = candidate.replace(
              /(?:'ll|'re|'ve|'d|'m|'s)$/g,
              "",
            );
        ...
        function normalizeSyllablesInLine(line) {
          return line.replace(
            ...
            (full, boundaryA, syllableA, boundaryB, fused) => {
              ...
              const syllable = (
                syllableA ||
                (fused ? fused.slice(0, 2) : "") ||
                ""
              ).toLowerCase();
              if (!syllable) return full;

              // Count total syllables
              const matches = (full.match(new RegExp(syllable, "gi")) || [])
                .length;
      ...
      x = x.replace(/\btry(?:in'?|i?n+a?|i?n'?na?|i?n'?n?a?)\b/gi, "tryna");
      ...
      // Add comma after "thank you" only when followed by a same-line address/name.
      x = x.replace(
        /\b(thank you)[ \t]+([A-Za-z][A-Za-z'’-]*)\b/gi,
        (full, ty, name, offset, str) => {
          if (isThankYouNounPhrase(str, offset)) return full;
          if (!isThankYouAddress(name)) return full;
          const casedName = normalizeThankYouName(name);
          if (ty === ty.toUpperCase()) {
            return `THANK YOU, ${casedName.toUpperCase()}`;
          }
          if (ty[0] === ty[0].toUpperCase()) {
            return `Thank you, ${casedName}`;
          }
          return `thank you, ${casedName}`;
        },
      );
      x = x.replace(
        /\b(thank you),[ \t]+([A-Za-z][A-Za-z'’-]*)\b/gi,
        (full, ty, follower) => {
          if (!THANK_YOU_BLOCKED_FOLLOWERS.has(follower.toLowerCase()))
            return full;
          const next =
            ty === ty.toUpperCase()
      ...
      x = x.replace(
        /\b((?:a|an|the|this|that|my|your|our|his|her|their)[ \t]+thank you),[ \t]+/gi,
        "$1 ",
      );
      ...
      if (preservedStandaloneParens.length > 0) {
        const restoreRe = new RegExp(
          `${STANDALONE_PAREN_SENTINEL}(\\d+)__`,
          "g",
        );
      ...
      x = x.replace(
        /(#(INTRO|VERSE|PRE-CHORUS|CHORUS|BRIDGE|HOOK|OUTRO)\s*\n\s*)+#\2/gi,
        "#$2",
      );
      x = x.replace(/(#HOOK\s*\n\s*)+#HOOK/gi, "#HOOK");
      ...
      x = x.replace(
        /(^|\n)(\([^\n]*?\))[ \t]*([A-Z][^\n]*)/g,
        function (match, boundary, bv, tail) {
    ...
    function computeBottomOffset(el) {
      ...
      for (const node of elements) {
        ...
        const candidate =
          bottom + rect.height * RAISE_BY_FACTOR + BUTTON_GAP_PX;
    ...
    function ensureShortcutListeners() {
      ...
      if (uiDocument && uiDocument !== document)
        bindShortcutListener(uiDocument);
    }
    ...
    function createFloatingButton() {
      ...
      if (!formatBtn) {
        ...
        formatBtn.innerHTML =
          "<span style=\"font-family: 'Times New Roman', serif; font-style: italic; font-weight: 700; font-size: 38px; line-height: 1; padding-bottom: 4px; padding-right: 2px;\">f</span>";
      ...
      if (!formatBtn.dataset.mxmStyled) {
        ...
        formatBtn.addEventListener("mouseenter", () => {
          formatBtn.style.transform = "translateY(-3px) scale(1.05)";
          formatBtn.style.boxShadow = `0 6px 22px ${MF_GLOW}`;
        });
        formatBtn.addEventListener("mouseleave", () => {
          formatBtn.style.transform = "";
          formatBtn.style.boxShadow = `0 4px 14px ${MF_GLOW}`;
        });
        formatBtn.addEventListener("focus", () => {
          formatBtn.style.boxShadow = `0 0 0 3px ${MF_GLOW}`;
        });
      ...
      if (!revertBtn.dataset.mxmStyled) {
        ...
        revertBtn.addEventListener("mouseenter", () => {
          ...
          revertBtn.style.boxShadow = "0 6px 16px rgba(0,0,0,.4)";
        });
        ...
        revertBtn.addEventListener("focus", () => {
          revertBtn.style.boxShadow = `0 0 0 2px ${MF_GOLD}`;
        });
    ...
    function updateRevertButtonState() {
      ...
      if (!btn) return;
      const hasStored =
        lastFormatState !== null || readStoredOriginal() !== null;
      ...
      chromeStorage.sync.get(
        ...
        (data) => {
          ...
          applyExtensionOptions({
            ...
            fixBackingVocals:
              payload.mxmBV ?? extensionDefaults.fixBackingVocals,
            showFloatingButton:
              payload.mxmButton ?? extensionDefaults.showFloatingButton,
          });
        },
      );
    ...
    function runFormat(passedOptions) {
      if (passedOptions && typeof passedOptions === "object") {
        ...
        if (
          ...
          updates.showFloatingButton = Boolean(
            passedOptions.showFloatingButton,
          );
        if (Object.keys(updates).length) applyExtensionOptions(updates);
      }
  ...
  // ============================================================================
  // END: PRESERVED FORMATTER LOGIC
  // ============================================================================

  // 2. Runtime Bridge: Allows the Extension Popup or Background script to trigger
  // the formatter cleanly via standard Chrome APIs.
  if (
    typeof chrome !== "undefined" &&
    chrome.runtime &&
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request.action === "runFormat") {
        const event = new CustomEvent("mxmFormatRequest", {
          detail: request.options || null,
        });
        document.dispatchEvent(event);
      }
    });
