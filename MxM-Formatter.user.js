...
(function (global) {
  ...
  if(window.top===window){
    const BTN_ID='mxmFmtBtn';
    let keybindingAttached=false;
    ...
    whenBodyReady(body=>{
      ensureButton(body);
      const obs=new MutationObserver(()=>ensureButton(body));
  ...
  function toast(msg){
    ...
    Object.assign(t.style,{background:'rgba(17,17,17,.95)',color:'#eaeaea',border:'1px solid #333',borderRadius:'10px',padding:'8px 10px',fontFamily:'system-ui',fontSize:'12px',position:'fixed',right:'16px',bottom:'80px',zIndex:2147483647});
    t.textContent=msg;
    host.appendChild(t);
